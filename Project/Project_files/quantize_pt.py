...
# Argument parser
parser = argparse.ArgumentParser(description='pytorch.org/docs/stable/quantization.html')
...
model = MobileNetv1()
model_str = "mobilenetv1_quant"

# Preparation for quantization aware training
print('prepare for qat')
model.eval()
...
path = f'ckpt/post_train_{model_str}.pt'
...
# Move everything to CPU for testing quantization
...
with torch.no_grad():
    for batch_idx, (images, labels) in enumerate(test_loader):
        ...
        test_total += labels.size(0)
        test_correct += predicted.eq(labels).sum().item()
print('Int8 Test loss: %.4f Int8 Test accuracy: %.2f %%' % (test_loss / (batch_idx + 1), 100. * test_correct / test_total))
