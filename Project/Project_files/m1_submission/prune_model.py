...
# Argument parser
parser = argparse.ArgumentParser(description='EE361K project pruner for saved MobileNetV1')
parser.add_argument('--model',          type=str,                       help='model checkpoint name (within ckpt directory)')
parser.add_argument('--prune_metric',   type=str,       default='l1',   help='metric used for pruning')
parser.add_argument('--iter_steps',     type=int,       default=5,      help='the number of steps for pruning [only in Prune network mode]')
parser.add_argument('--finetune_epoch', type=int,       default=5,      help='the number of epochs for finetuning')
...
#create working dir for the pruning process
os.makedirs('pruned', exist_ok=True)


#load saved weights
ckpt = torch.load(f'ckpt/{args.model}.pt')
model = MobileNetv1()
...
def prune_network(model, metric, prune_ratio, iterative_steps=5, fine_tune_epochs=1, ignored_layers=None):
    ...
    torch.onnx.export(model.to('cpu'),
                      ...
                      'onnx_model/{}_{}_{}_{}_{}_pruned_net.onnx'.format(
                          args.model, metric, prune_ratio, iterative_steps, fine_tune_epochs),
